...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def treeItemClicked(self, item, col):
        ...
        if not os.path.exists('ligTabVisual'): os.mkdir('ligTabVisual')
        folder = 'ligTabVisual' 
        for filename in os.listdir(folder): #delete everything in ligTabVisual so it doesn't get filled
    ...
    def updateUserLigFolder(self,ligandList,ligPathList):
        #delete all ligands in userLigands (a folder separate from the ligands inside of mason)
        if not os.path.exists('Mason\\userSelectedLigands'): os.mkdir('Mason\\userSelectedLigands')
        folder = 'Mason\\userSelectedLigands'
    ...
    def updateConfigFile(self, templateName,nCoordNum, nCoreNum, nCharge, nMultiplicity, nFrozen):
        ...
        for line in iF:
            ...
            if 'coordNum =' in strippedLine:
                ...
                newLine = strippedLine.replace(strippedLine, 'multiplicity = ' + nMultiplicity)
        ...
        writingFile = open('Mason\\config.ini', 'w')
