...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def treeItemClicked(self, item, col):
        ...
        self.ligShownEdit.setText(item.text(col)) #shows what ligand is selected
    ...
    def addLigandButtonPushed(self):
        ...
        if self.smilesLineEdit.text() == '' or self.ligShownEdit.text() == '' or self.bondingAtoms.text() == '':
            ...
            #if the user clicks "ok" then add the ligand, if they exit then do nothing
        msg.setText(message)
        x = msg.exec_()
    ...
    def updateUserLigFolder(self,ligandList):
        ...
        os.makedirs('Mason Files Generated By GUI\\' + templateName) #make a folder for all the ligands the user selected
        #for each ligand, copy it and paste it into the user folder
        for ligand in ligandList:
            if ligand=="ligands": #folder is named ligands, so skip since it's not a file
                continue
            shutil.copyfile('Smarties\\ligands\\'+ligand,'Mason Files Generated By GUI\\'+ templateName + '\\' + ligand) #(source, destination)
        #tell a user if all ligands have been selected?? sub libraries of ligands??
        return templateName
    ...
    def replaceAtoms(self):
        ...
        if not self.isEmpty(nCoordNum, nCoreNum, nCharge, nMultiplicity, nFrozen):
            #if everything is filled in, popup message asking if everything is correct
            msg = QMessageBox()
        ...
        ligandList = self.ligandsSelected() #get the ligands that the user selects
        templateName = self.updateUserLigFolder(ligandList)

        #next section updates the config.ini file in Smarties with inputs from the gui 
        newFileContent = ''
        ...
        writingFile = open('Smarties\config.ini', 'w')
