...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def updateUserLigFolder(self,ligandList):
        ...
        for ligand in ligandList:
            if ligand=="ligands": #folder is named ligands, so skip since it's not a file
                continue
            shutil.copyfile('Smarties\\ligands\\'+ligand,'Smarties\\userLigands\\'+ligand) #(source, destination)
    ...
    def replaceAtoms(self):
        ...
        #get the ligands that the user selects
        ligandList = self.ligandsSelected()
        self.updateUserLigFolder(ligandList)
