...
def AddLigands2Mol(mol, ligands, config):
    ...
    for ligand in ligands:
        mol = AddLigand2Mol(mol, ligand, constraints, config)
    ...
    # print((mol.NumAtoms() + ligand.NumAtoms()))
        ...
        forcefield.SteepestDescent(stepsPerAtom * mol.NumAtoms())
        # print(forcefield.Energy())
        forcefield.GetCoordinates(mol)
        forcefield.Setup(mol, constraints)
        forcefield.WeightedRotorSearch(mol.NumAtoms(), stepsPerAtom * mol.NumAtoms())
        forcefield.GetCoordinates(mol)
        forcefield.Setup(mol, constraints)
        # print(forcefield.Energy())
        forcefield.ConjugateGradients(stepsPerAtom * mol.NumAtoms())
        # print(forcefield.Energy())
        forcefield.GetCoordinates(mol)
